...
import { formatActivityType, formatActivityTimestamp, formatLogDetails } from "../../utils/index.js";
import ErrorBoundary from "../ErrorBoundary.jsx";

function ActivityLogList({ activityLog, clearHistory }) {
  return (
    <>
          ...
          {activityLog.map((entry) => (
            ...
            >
              ...
              <p className="text-gray-800">
                {typeof entry.details === "string" ? entry.details : formatLogDetails(entry.details)}
              </p>
              {(entry.searchCode ?? entry.search_code) && (
...
export default function DashboardHome({ activityLog, boxes, clearHistory }) {
  const filteredActivityLog = Array.isArray(activityLog)
    ? activityLog.filter(
        (entry) => entry.type === "checkout" || entry.type === "return"
      )
    : [];
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/20 p-5 md:p-6 shadow-lg shadow-emerald-100/50 hover:shadow-xl hover:shadow-emerald-200/50 transition-all duration-300">
            ...
            <div className="space-y-3">
              <div className="bg-white rounded-2xl px-5 py-4 shadow-md hover:shadow-lg hover:bg-emerald-50/40 transition-all duration-300">
                <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-700 mb-2">Registered Boxes</p>
                <p className="text-3xl font-bold bg-gradient-to-r from-emerald-700 to-sky-700 bg-clip-text text-transparent">
                  {boxes.length}
                </p>
              </div>
            </div>
          ...
          {/* Activity Feed – only show checkout/return events */}
          <div className="rounded-3xl bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/20 p-5 md:p-6 max-h-[28rem] overflow-y-auto custom-scrollbar shadow-lg shadow-emerald-100/50 hover:shadow-xl hover:shadow-emerald-200/50 transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              ...
              {filteredActivityLog.length > 0 && clearHistory && (
                <button
                  type="button"
            ...
            <ErrorBoundary
              fallback={
                <p className="text-xs text-amber-700">
                  Activity log could not be displayed. Try clearing history or reloading.
                </p>
              }
            >
              <ActivityLogList activityLog={filteredActivityLog} clearHistory={clearHistory} />
            </ErrorBoundary>
          </div>
        </div>
      </div>
