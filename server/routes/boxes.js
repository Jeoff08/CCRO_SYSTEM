...
router.get("/", (req, res) => {
  try {
    const boxes = db.prepare("SELECT * FROM boxes ORDER BY box_number, bay, shelf, row").all();
...
router.put("/:id", (req, res) => {
  try {
    ...
    const updatedBox = db.prepare("SELECT * FROM boxes WHERE id = ?").get(req.params.id);
